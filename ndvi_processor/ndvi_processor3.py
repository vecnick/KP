...
class NDVIProcessor2:
    ...
    def select_worker(self, chunk_id: str) -> str:
        """
        Выбор наименее загруженного воркера для обработки чанка
        """
        workers_info = self.client.scheduler_info()['workers']
        if not workers_info:
            raise RuntimeError("No workers available")
        # Для каждого воркера считаем количество активных задач
        worker_load = {}
        for addr, info in workers_info.items():
            n_tasks = len(info.get('processing', {}))
            worker_load[addr] = n_tasks
        # Выбираем воркера с минимальной загрузкой
        selected_worker = min(worker_load, key=worker_load.get)
        self.logger.info(f"Selected least loaded worker {selected_worker} (tasks: {worker_load[selected_worker]}) for chunk {chunk_id}")

...
if __name__ == '__main__':
    ...
    def scale_data(data):
        # Проверяем текущий диапазон
        min_val = data.min().compute()
        max_val = data.max().compute()
        logger.info(f"Исходный диапазон значений: [{min_val}, {max_val}]")
        
        # Данные уже откалиброваны (SR - Surface Reflectance)
        # Просто проверяем, что значения в разумном диапазоне
        if max_val > 1.0:
            logger.warning("Значения превышают 1.0, что необычно для SR данных")
        if min_val < 0.0:
            logger.warning("Отрицательные значения обнаружены, что необычно для SR данных")
            
        return data

    # Масштабируем данные
    b4_data = scale_data(b4_data)
    b5_data = scale_data(b5_data)

    # Создаем маску для валидных данных
    valid_mask = (b4_data > 0.01) & (b5_data > 0.01) & (b4_data < 1.0) & (b5_data < 1.0)
    
    # Применяем маску к данным
    b4_masked = b4_data.where(valid_mask)
    b5_masked = b5_data.where(valid_mask)

    # После загрузки данных B4 и B5, перед масштабированием, добавляем визуализацию гистограмм
    logger.info("Визуализация гистограмм B4 и B5...")
    plt.figure(figsize=(12, 6))
    plt.subplot(1, 2, 1)
    b4_values = b4_masked.values.flatten()
    ...
    plt.legend()
    plt.subplot(1, 2, 2)
    b5_values = b5_masked.values.flatten()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('histograms_b4_b5.png')
    plt.close()
    logger.info("Гистограммы B4 и B5 сохранены в histograms_b4_b5.png")
    ...
    # Создаем маску для избежания деления на ноль и отрицательных значений
    valid_data = (b4_data >= 0) & (b5_resampled >= 0) & ((b5_resampled + b4_data) > 0)

    # Вычисляем NDVI с улучшенной маской
    ndvi = xr.where(
        valid_data,
    ...
    # Создание улучшенной маски для фильтрации невалидных значений NDVI
    logger.info("Создание маски для NDVI...")

    # Более мягкая маска для NDVI
    ndvi_mask = (ndvi >= -1) & (ndvi <= 1)  # Теоретически возможный диапазон NDVI

    # Более мягкая маска для исходных данных
    data_mask = (b4_data >= 0) & (b5_resampled >= 0)  # Убираем строгое условие > 0.01
